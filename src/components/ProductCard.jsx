export default function ProductCard({ product, isFeatured, onToggle }) {
  return (
    <div className={`relative bg-white rounded-2xl shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden flex flex-col group border-2 ${isFeatured ? 'border-yellow-400' : 'border-transparent'}`}>
      ...
      <div className="h-44 sm:h-52 flex items-center justify-center p-4 sm:p-6 bg-gray-50">
        <img
          src={product.image}
          alt={product.title}
      ...
      <div className="p-3 sm:p-4 flex flex-col flex-1 gap-1.5">
        <span className="text-xs uppercase tracking-widest text-indigo-400 font-semibold truncate">
          {product.category}
        </span>
        ...
        <button
          ...
          className={`mt-2 w-full py-2 sm:py-2.5 rounded-xl text-xs sm:text-sm font-semibold transition-all duration-200 cursor-pointer
            ${isFeatured
